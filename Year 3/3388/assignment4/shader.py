class shader:
    ...
    def __shadowed(self, object, I, S, objectList):
        ...
        for k in objectList:
            ...
            Ik = matrixInverse * I
            # compute S = matrixInverse * S and normalize S.
    ...
    def __init__(self, intersection, direction, camera, objectList, light):
        ...
        self.__R = self.__N.scalarMultiply(
            2 * (self.__S.removeRow(3).transpose() * self.__N.removeRow(3)).get(0, 0)) - self.__S
        # compute vector to center of projection V = Te - I
        self.__V = (self.__Te - self.__I).normalize()
        # compute Id = max{N * S, 0} and Ix = max{R * V ,0}
        ...
        if self.__shadowed(self.__object, self.__I, self.__S, objectList):
            self.__f = self.__r[0]
        # if not compute f = r[0] + r[1]* Id + r[2] * Is ** r[3]
        else:
