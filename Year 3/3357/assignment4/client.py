...
def main():
    ...
    # Send a second request to the server if moved

    if response_list[1] == '301':
        print("Re-directing the client.")
        print(response_line)
        while (not headers_done):
            header_line = get_line_from_socket(client_socket)
            header_list = header_line.split(' ')
            print(header_line)
            if(header_list[0] == "Location:"):
                clientRedirect(header_list[1])
...
# When the file has been moved, run the redirect method to download at new location

def clientRedirect(url):
    try:
