        ...
        if request_list[0] == 'Time':
            ...
            # If we did not get the proper HTTP version respond with a 505.
                ...
                # File exists, so prepare to send it!
                    ...
                    serverFileTime = os.path.getmtime(req_file)
                    cacheFileTime = float(request_list[1])

                    if serverFileTime - cacheFileTime > 0:
                        send_response_to_client(conn, '200', req_file)
                    else:
                        print('Requested file has not been modified')
