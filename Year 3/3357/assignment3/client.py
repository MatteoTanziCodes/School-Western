...
class CacheException(Exception):
    print('Error with cacheHost and cachePort values')
...
def main():
    ...
    parser.add_argument("-proxy", "--cache", help="Connects to Cache")
    ...
    if args.cache:
        try:
            ...
            cacheHost = sys.argv[2].split(":")[0]
            cachePort = int(sys.argv[2].split(":")[1])
        except ValueError:
        ...
        try:
            ...
            client_socket.connect((cacheHost, cachePort))
        ...
        file_processing(client_socket, serverHost, serverPort, file_name)
        ...
        # The connection was successful, so we can prep and send our message.
        file_processing(client_socket, host, port, file_name)
